...
import { RedditUser, GitHubUser } from '@common/interface';
...
import { Session } from './social-auth.schema';
...
async function getRedditUser({ code }: { code: string }): Promise<RedditUser> {
  ...
  const session = await axios
    ...
    .catch(error => {
      ...
      Logger.error(`Error getting the Reddit user session with message: ${message}`);
...
export class SocialAuthService {
  ...
  async redditAuth(input, context) {
    ...
    let user = await this.userModel
      ...
      .then(data => data)
      .catch(error => {
        Logger.error(`Error getting user from database with redditId ${redditUser.id}`);
        throw error;
      });
  ...
  async githubAuth(input, context) {
    ...
    let user = await this.userModel
      ...
      .then(data => data)
      .catch(error => {
        Logger.error(`Error getting user from database with githubId ${githubUser.id}`);
        throw error;
      });
    ...
    if (user) {
      await user.update({
        ...githubUser,
        // Fields to not overwrite
        name: get(user, 'name', null) || get(githubUser, 'name', get(githubUser, 'login', null)),
    ...
    if (!user) {
      user = await this.userModel
        .create({
          ...
          active: true,
          avatar: githubUser.avatar_url,
          name: get(githubUser, 'name', get(githubUser, 'login', faker.name.findName())),
        })
        .then(data => {
          return data;
        })
  ...
  async googleAuth(input, context) {
    ...
    let user = await this.userModel.findOne({ googleId: String(googleUser.id) }).exec();

    if (user) {
      await user.update({
        ...googleUser,
        // Fields to not overwrite
        name: get(user, 'name', null) || `${googleUser.given_name} ${googleUser.family_name}`,
